...
- ⚠️ **КНОПКА «📰 Дайджест» (= команда «дайджест») = ТОЛЬКО персональный дайджест** (`calendar/digest.py`): RSS (РБК, Коммерсантъ) + TG-каналы (Медуза, Важные истории, The Bell, BAZA) + курс ЦБ, отправка владельцу через бота. НЕ расписание, НЕ web_search, НЕ Google News. Саммари — подробное, без искажения контекста (правка 15.08).
