...
- Значения — в `~/.openclaw/.env` (его читают И CLI, И gateway — одного systemd drop-in НЕ хватает: CLI в SSH-сессии падал с `gateway.auth.token resolved to an empty or non-string value`) и дублируются в `/root/.openclaw/secrets.env` (600) через systemd drop-in `20-secrets-env.conf` (`EnvironmentFile=-`, как `relaymodels.env`), НЕ система `config set --ref-provider`. Бэкап: `~/.openclaw/openclaw.json.pre-secretref.*`. Проверено: systemd отдаёт все 8 переменных дочернему процессу, `openclaw config validate` чист с загруженным env. <!-- project: github.com/Dmgromov01/openclaw-workspace -->
