...
## Память (жёсткое правило)

**Перед ответом на любой вопрос о прошлых событиях, предпочтениях пользователя, предыдущих задачах или принятых решениях — ТЫ ОБЯЗАН использовать memory_search и/или прочитать файлы memory/. Никогда не угадывай прошлый контекст.**

- Вопрос «что мы делали на прошлой неделе?» → `memory_search` + чтение дневников.
- Вопрос «какие были решения по X?» → поиск в memory/, decisions.md, MEMORY.md.
- Не помнишь → скажи «не нашёл в памяти» и предложи поискать вместе, а не выдумывай.
- Галлюцинация прошлого контекста = критическая ошибка.
