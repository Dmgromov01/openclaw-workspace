...
def _summarize_source(name: str, posts: list) -> str:
    ...
    try:
        ...
        payload = json.dumps({
            ...
            "messages": [
                {"role": "system", "content": (
                    "Ты — редактор дайджеста. Перед тобой посты из ОДНОГО источника за последние 3 часа."
                    " Составь КРАТКОЕ саммари на русском: максимум 3-4 предложения, только суть."
                    " СОХРАНИ все ключевые факты и цифры из оригинала (кто, что, где, сколько) —"
                    " смысл не искажай, ничего не выдумывай. ЖЁСТКО урежь воду, повторы и детали,"
                    " не влияющие на смысл. Не перечисляй посты списком — связный сжатый пересказ."
                )},
