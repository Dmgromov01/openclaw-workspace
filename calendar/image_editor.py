...
def process_ai_photo(input_path: str, output_path: str, user_instruction: str = "") -> str:
    """
    Редактирование/рестайлинг фото через Pollinations (бесплатно, без ключей).
    Внимание: Pollinations генерирует по промпту (нет true img2img),
    поэтому используем описание/инструкцию + стиль как промпт.
    """
    """
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 3:
        print("Использование: python3 image_editor.py '<инструкция>' <output.jpg>")
        sys.exit(1)
    out = process_ai_photo(None, sys.argv[2], sys.argv[1])
    print(out)
