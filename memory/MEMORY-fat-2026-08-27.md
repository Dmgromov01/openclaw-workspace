...
- **hub.gbkz.uk** — R2D2 Personal AI Hub (TanStack Start SSR, 127.0.0.1:8091, systemd-юнит r2d2-hub). Развёрнут 27.08.2026: A `hub` → 138.124.180.178 (Cloudflare, proxied), nginx sites-available/hub.gbkz.uk (прокси :8091, HTTP→HTTPS 301), TLS Let's Encrypt до 25.11.2026. Vite `preview.allowedHosts` += hub.gbkz.uk (иначе 403 «Blocked request»). certbot починен 27.08: pyOpenSSL 26.4 + certbot 5.7.0 через pip (--break-system-packages; apt-версия падала с lib.GEN_EMAIL / X509Req). nginx не в PATH → запускать certbot с `PATH=/usr/sbin:$PATH`. BotFather Mini App URL = https://hub.gbkz.uk/ (за Дмитрием).
...
## 🧠 ГЛАВНЫЙ ПРИНЦИП (25.08, требование Дмитрия): думать о НАМЕРЕНИИ, а не исполнять буквально
- Дмитрий: «надо чтобы ты помнил контекст и сам логично догадывался что делать а что нет» — это про ВСЮ логику действий и принятия решений, не про один случай.
- **Что это значит на практике:**
  1. **Контекст важнее команды.** Прежде чем действовать — спросить себя: что Дмитрий РЕАЛЬНО хочет? Что уже есть/сделано? Каков объём?
  2. **«Попробуй модель X» = тест на новой задаче**, а не массовая перегенерация существующих ассетов (урок 10:32 с krea: я запустил 20 фонов, а нужно было одну картинку).
  3. **Проверять идемпотентность перед действием**: если результат уже есть — не переделывать.
  4. **Стоимость/риск**: если операция дорогая или массовая, а намерение неочевидно — уточнить ОДНИМ вопросом, прежде чем жечь деньги/время.
  5. **Не зацикливаться**: одно действие → один ответ; не повторять одно и то же.
- Действовать как самостоятельный разумный ассистент, а не буквальный исполнитель.
...
- **Модели**: primary `deepseek/deepseek-v4-flash` (прямой API, дёшево 1M ctx). Провайдеры в конфиге (19.08, ключи восстановлены): **deepseek** · **openrouter** (auto, qwen/qwen-image-3-pro, + openai/text-embedding-3-small с 24.08) · **google** (gemini-3.1-flash-image-preview, gemini-2.5-flash) — все с apiKey. **DeepSeek НЕ принимает картинки** — для image использовать Gemini/OpenRouter. Ключи: `/root/.openclaw/credentials/` (openrouter.key, ai-studio.key, github.token, telegram-app.json, jina.key — все chmod 600, ВНЕ git). ⚠️ Схема моделей провайдера не принимает `output`/`input:["text","image"]` — только `input:["text"]` (+api/contextWindow/maxTokens).
  ⚠️ **26.08: Анализ фото = `deepseek/deepseek-v4-flash-vision-exp`** через ПРЯМОЙ DeepSeek API (модель есть в /models; ключ — из openclaw.json, файл credentials/deepseek.key был битый/старый!). Прописан в `models.providers.deepseek.models` (с `mediaInput.image`) + `tools.media.models[0]` (capabilities: image, prompt на русском). Проверено: 401-а нет.
  ⚠️ **26.08: Генерация фото = `openrouter/google/gemini-3.1-flash-image-preview`** (primary, через OpenRouter, fallbacks пустые — grok убран). ⚠️ Встроенный `image_generate` для OpenRouter ходит chat-completions путём — grok-imagine-image-2.0 там НЕ работает (404 «No endpoints found»). Grok-генерация только через скрипт `scripts/gen_grok.py` (OpenRouter /api/v1/images, $0.07/шт). План Дмитрия: когда кончатся средства на OpenRouter — перейти на прямой xAI Imagine ($0.02/шт, нужен XAI_API_KEY). ⚠️ **Урок 24.08**: memorySearch требует, чтобы embedding-модель была в каталоге провайдера, иначе векторная часть гибрида молча не работает. OpenRouter не показывает embedding-модели в GET /models, но POST /v1/embeddings работает.
...
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:16:17 -->
- 🤖 Аватар/маскот ассистента (07:06–07:09): По правилу 19.08 генерации с нуля → Pollinations (`python3 calendar/image_gen.py "<промпт>" [w] [h]`).; Дмитрию не понравились: эмблема-щит → «маскот» → «робот ИИ»; потом ошибка «image dimensions are invalid. Must be 640x360» — перегенерил 640x360. Итог: gen_1787209783.jpg (робот ИИ, 640x360) отправлен. [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:16-17]
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:7:10 -->
- 🎾 Падел: спорные моменты «корт vs стекло» (05:24–05:54): Дмитрий спорил: можно ли судить попадание в стекло по углу отскока. Моя позиция (подтверждена): НЕТ — отражение от вертикального стекла сохраняет вертикальную компоненту скорости; направление отскока задаёт фаза траектории, а не факт касания корта.; По его запросу поднял официальные правила FIP (Rules of Padel, ред. Acapulco 28.11.2025, PDF с padelfip.com):; Rule 12.3: мяч, коснувшийся любой части корта (вкл.... [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:7-10]
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:20:20 -->
- 🛠️ MiniApp (Personal Hub) — аудит + починка (07:38–08:36) — ГЛАВНОЕ: Дмитрий: «приложение MiniApp запускается кнопкой OpenClawHub прямо в боте, делал через Gemini-чат; посмотри код/качество; карта погоды показывает Европу — нужна Москва/МО; не работает дайджест; непонятно добавление/удаление; проверь весь функционал». [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:20-20]
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:29:31 -->
- Что было сломано: Карта: Leaflet setView zoom 7 → видна вся Европа; погода — ХАРДКОД (24°C «Ясно»), реального API не было.; Дайджест: `digestItems` в static/index.html **не объявлен вообще** → экран падал; бэкенда не было.; Источники: только localStorage браузера, ни на что не влияли. [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:29-31]
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:23:26 -->
- Архитектура (запомнить): **Активная версия**: `miniapp/static/index.html` (React+Babel+Tailwind+Leaflet). Старый `miniapp/index.html` — легаси, НЕ раздаётся.; Сервер: `python3 -m http.server 8080 --directory /root/openclaw/miniapp/static` → заменён на **`miniapp/server.py`** (порт 8080, тот же туннель).; Публикация: cloudflared tunnel → **https://equipment-rrp-states-awesome.trycloudflare.com** (временный URL! при рестарте cloudflared меняется → кнопка в боте ломается). Кнопка ставится `update_bot_button.py` / `bot/miniapp_menu.py`.; ⚠️ MiniApp — отдельный сервис (не OpenClaw прод), правится напрямую без правила релиза. [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:23-26]
<!-- openclaw-memory-promotion:memory:memory/2026-08-20.md:34:37 -->
- Что сделано: **`miniapp/server.py`** (написан с нуля): статика + API:; `GET /api/digest` → {blocks:[{title,posts}], curs, total, ts}, кеш 15 мин, собирает посты через calendar/digest.py (`_channel`, `_rss`, `_curs`).; `GET/POST/DELETE /api/sources` → sources.json (дефолт: Медуза, Важные истории, The Bell, BAZA, РБК, Коммерсантъ).; Перезапуск: убить старый http.server, `nohup python3 miniapp/server.py 8080 > miniapp_server.log 2>&1 &` (процесс был pid 103908). [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-20.md:34-37]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:3:6 -->
- 06:53 — Миниапп R2D2 упал (502 Bad Gateway): Симптом: скрин Дмитрия — Cloudflare 502, Host Error (gbkz.uk), 06:53 UTC.; Причина: nginx → /miniapp/ → 127.0.0.1:8080, а python-сервер миниаппа (server.py) умер (запущен вручную nohup, НЕТ systemd-юнита/авторестарта). nginx и гейтвей были живы.; Фикс: перезапуск `nohup venv/bin/python miniapp/server.py >> miniapp_server.log 2>&1 &` → порт 8080 слушает, /miniapp/ = 200.; OOM не найден в journalctl; просто умер. Надо завести systemd-юнит для авторестарта (предложено Дмитрию). [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-21.md:3-6]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:9:11 -->
- 06:57 — systemd-юнит для миниаппа (по «да» Дмитрия): Создан `~/.config/systemd/user/miniapp.service` (копия в services/miniapp.service): python server.py на :8080, Restart=always, RestartSec=5, лог append в miniapp_server.log, enabled.; Работает: active (running), порт 8080 слушает, /miniapp/ → 200 локально и публично.; Больше не умрёт молча: автоподъём при падении и при ребуте (linger=yes). [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-21.md:9-11]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:14:17 -->
- 07:01 — Календарь в миниаппе не работал (500): Симптом: /api/calendar → {"error": "No module named 'google'"} 500.; Причина: миниапп поднят через venv/bin/python, а google-библиотек в venv НЕ было (system python3 их имел — раньше сервер, видимо, запускался системным python).; Фикс: `venv/bin/pip install google-api-python-client google-auth google-auth-oauthlib google-auth-httplib2`, restart miniapp.service.; Проверено: /api/calendar → 200, события отдаются (сегодня «Дегустация вина» 18:00). [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-21.md:14-17]
<!-- openclaw-memory-promotion:memory:memory/2026-08-21.md:24:27 -->
- 11:10–11:25 — Clawpatch: установка, ревью, фиксы, рутина: security high: токен через argv (виден в ps/history) — УБРАНО; test-gap: get_token() без тестов — добавлен test_update_bot_button.py (11 тестов, OK); bug high: исключения чтения .env/config.json и Bot() вне try/except — ОБЕРНУТЫ; maintainability: хардкод /root/openclaw в путях — сделано script-relative + fallback, get_token(paths=None) [score=0.828 recalls=0 avg=0.620 source=memory/2026-08-21.md:24-27]
