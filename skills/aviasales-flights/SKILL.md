...
## Безопасный запуск и проверка доступа

Название сервиса: **Travelpayouts** (не `travellayouts` и не `TravelLayouts`). В этом серверном окружении токен уже хранится в `/root/.openclaw/.env` под именем `TRAVELPAYOUTS_TOKEN`; файл имеет права `600`.

Никогда не читай и не показывай `/root/.openclaw/.env`: запрещены `cat`, `grep`, `printenv`, `env`, `echo` и `curl` с токеном. Не вставляй токен в URL, сообщения, логи или JSON-ответ.

Для выполнения используй только allowlisted wrapper:

```bash
/root/openclaw/bin/aviasales-flights-operator --lookup "Istanbul"
/root/openclaw/bin/aviasales-flights-operator --from MOW --to LED --date 2026-04-15 --limit 3
```

Wrapper сам загружает `.env` в окружение дочернего процесса и запускает фиксированный `scripts/search_flights.py`. Не запускай `python3` напрямую и не пытайся диагностировать секрет вручную.

Для проверки доступа используй небольшой IATA lookup (токен не нужен), затем запрос с реальными параметрами пользователя. Успех означает валидный JSON с `results`/`count`; отсутствие токена или HTTP/API-ошибка должны сообщаться только как тип ошибки и HTTP-статус, без URL с query-параметрами и без токена.

Если пользователь просит «проверить доступ к Travelpayouts», сначала выполни lookup, затем один короткий запрос с указанным маршрутом/датой. Не называй это Travelpayouts «TravelLayouts».
